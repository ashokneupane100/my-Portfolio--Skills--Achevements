      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-20">
        ...
        <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center">
          {cardItem.map(({ id, logo, name }) => (
            <div
              key={id}
              className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition duration-300 flex flex-col items-center"
            >
              <img
                src={logo}
                alt={name}
                className="w-40 h-40 object-cover object-center rounded-full mt-4"
              />
